...
export default function DashboardLayout() {
  ...
  return (
    <div className="app-layout" style={{ minHeight: '100vh', position: 'relative' }}>
      ...
      {/* 📱 FLOATING BOTTOM NAVIGATION (Mobile only, Hidden on Desktop via CSS) */}
      <nav className="bottom-nav desktop-hide">
        ...
        {/* 🚀 ELECTRIC BLUE FAB (Central Action) */}
        <Link to="/observasi/baru" style={{ textDecoration: 'none' }}>
            <div className="fab-center">
                <Plus size={32} color="white" />
            </div>
        </Link>

        <Link to="/eksplorasi" className={`nav-item ${isActive('/eksplorasi') ? 'active' : ''}`}>
           <GraduationCap size={22} color={isActive('/eksplorasi') ? 'var(--primary)' : 'var(--text-muted)'} />
           <span>Sentra</span>
        </Link>
