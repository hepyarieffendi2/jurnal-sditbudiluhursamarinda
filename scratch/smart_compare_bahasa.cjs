...
// Flat list of all DB levels in BI subareas
...
// For each user level, find matching DB level (manual intelligent mapping)
const mappings = [];
...
function findDbMatch(userLvlText) {
  ...
  // Custom manual rules based on semantic matches
  for (const dbLvl of allDbLevels) {
    ...
    if (lower.includes("sandpaper letters") && dbLabelLower.includes("sandpaper") && dbLabelLower.includes("cursive")) {
      return dbLvl; // Menulis Cursive
    }
    if (lower.includes("movable alphabet") && dbLabelLower.includes("movable alphabet") || (lower.includes("movable alphabet") && dbLabelLower.includes("alphabet bergerak"))) {
      return dbLvl;
    }
    ...
    if (lower.includes("awalan") && dbLabelLower.includes("imbuhan")) {
      return dbLvl; // Awalan matches Studi Imbuhan
    }
    if (lower.includes("akhiran") && dbLabelLower.includes("imbuhan")) {
      return dbLvl; // Akhiran also matches Studi Imbuhan
    }
...
console.log("\n=================== UNMATCHED DB LEVELS (LEFT OUT OR NEED MERGE) ===================");
unmatchedDbLevels.forEach(lvl => {
  console.log(`- Sub-area: ${lvl.subAreaName} (${lvl.subAreaId}) | Level: ${lvl.label}`);
});
